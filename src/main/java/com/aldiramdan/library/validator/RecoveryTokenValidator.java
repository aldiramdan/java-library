...
public class RecoveryTokenValidator {
    ...
    public void validateRecoveryTokenAlreadyConfirm(Optional<RecoveryToken> findToken) throws Exception {
        if (Objects.nonNull(findToken.get().getConfirmedAt())) {
            throw new NotProcessException("Account is already recovered!");
        }
    }

    public void validateRecoveryTokenAlreadyExpire(Optional<RecoveryToken> findToken) throws Exception {
